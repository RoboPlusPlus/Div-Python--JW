...
def crawler(startpage, maxpages=100, singledomain=True):
    ...
    print("All keywrd search results\n:")
...
def wordcount(soup):
    ...
    all_search_results.append(lookup_word("trump", words))
#------------------------------------------------------------------------------
def lookup_word(keyword, words):    
    search_result_list = []
    for i, word in enumerate(words, start=0):
        if word == keyword:
            for j in range(i-5, i+5):
                try:
                    search_result_list.append(words[j])
                except:
                    pass
    print("######################")
...
if __name__ == "__main__":
    START = default_timer()
    crawler('http://www.vg.no', maxpages=100, singledomain=True)
    END = default_timer()
